...
export type GetSuggestedTimeSlotsOptions = {
...
function detectTemporalSignals(raw: string): TemporalSignals {
  ...
  const mentionsMorning =
    text.includes(" manana ") || text.includes(" temprano ") || text.includes(" morning ");
  const mentionsAfternoon =
    text.includes(" tarde ") || text.includes(" afternoon ");
  const mentionsNight =
    text.includes(" noche ") || text.includes(" nocturno ") || text.includes(" night ");
...
function getFlexibility(
  ...
  if (
    ...
    /\b(cuando puedan|algun dia|algún día|esta semana|el otro finde)\b/.test(text)
  ) {
    return "high";
  }
...
function getDistancePenaltyForExplicitTemporalSignal(
  ...
  if (
    !(
      signals.mentionsWeekend ||
      signals.mentionsWeekday ||
      signals.mentionsTwoWeekends
    )
  ) {
    return 0;
  }
...
function getPreferenceShapeBonus(
  profile: SuggestionProfile,
  day: Date,
  hour: number,
) {
...
export function getSuggestedTimeSlots(
  ...
  const learnedProfile = options?.learnedProfile ?? null;
