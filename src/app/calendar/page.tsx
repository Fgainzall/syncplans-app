 ...
 * En UI puedes seguir mostrando "Pareja" si tu groupMeta usa "pair".
 */
 ...
 * Si tu UI (groupMeta) espera "pair", convertimos "couple" -> "pair" al renderizar.
 */
...
export default function CalendarPage() {
  ...
  // ✅ Dejamos ambos toggles por compat (UI "pair" + motor "couple")
  ...
  const refreshCalendar = useCallback(
    async (opts?: { showToast?: boolean; toastTitle?: string; toastSubtitle?: string }) => {
      ...
      try {
        ...
        // ✅ IMPORTANTÍSIMO: normalizamos a family | couple (NO pair)
        ...
        const enriched: CalendarEvent[] = (rawEvents || [])
          .map((ev: any) => {
            ...
            if (gid) {
              ...
              // Si viniera desde DB con algo raro
              gt = (ev.groupType ?? "personal") as any;
            }

            // ✅ NO re-serializar UTC
            ...
            return {
              ...
              groupType: normalizeForConflicts(gt), // ✅ clave
            } as CalendarEvent;
          })
  ...
  const filteredEvents = useMemo(() => {
    const isEnabled = (g?: GroupType | null) => {
      ...
      if (key === ("couple" as any)) {
        // si el usuario desactiva "pair" o "couple", respetamos ambos
  ...
  // ✅ Conflictos sobre TODOS (no visibles), usando groupType normalizado
  ...
  // ✅ Toggle simple y coherente
  const toggleGroup = (g: GroupType) =>
    setEnabledGroups((s: any) => {
      if (g === ("pair" as any)) {
        const next = !s.pair;
        // mantenemos couple alineado
...
function EventRow({
  ...
  // ✅ Para UI: convertimos couple -> pair si tu meta/label lo espera así
