...
function NewEventDetailsInner() {
  ...
  const [endLocal, setEndLocal] = useState(() => toInputLocal(addMinutes(initialStart, 60)));
  ...
  const [preflightDefaultChoice, setPreflightDefaultChoice] = useState<PreflightChoice>("edit");
    ...
    (async () => {
      ...
      try {
        ...
        if (typeParam === "group" && !groupIdParam) {
          const next = buildUrl("group", new Date(startDate).toISOString(), fallbackGroupId || null);
          router.replace(next);
        }
      } catch (err: any) {
        if (!alive) return;
  ...
  const theme = useMemo(() => {
    if (effectiveType === "group") {
      return {
        ...
        border: groupType === "family" ? "rgba(96,165,250,0.28)" : "rgba(248,113,113,0.28)",
        soft: groupType === "family" ? "rgba(96,165,250,0.14)" : "rgba(248,113,113,0.12)",
      };
    }
    return {
  ...
  const errors = useMemo(() => {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) e.push("Fecha/hora inválida.");
    if (endDate.getTime() <= startDate.getTime()) e.push("La hora de fin debe ser posterior al inicio.");
  ...
  const doSave = async (payload: {
    ...
    try {
      ...
      setToast({ title: "Evento creado ✅", subtitle: "Volviendo al calendario…" });
      window.setTimeout(() => router.push("/calendar"), 450);
    } catch (err: any) {
      setToast({ title: "No se pudo guardar", subtitle: err?.message || "Intenta nuevamente." });
  ...
  const preflight = async (payload: {
    ...
    const conflicts = computeVisibleConflicts(combined).filter((c) => c.incomingEventId === incomingId);
          ...
          <div style={styles.row}>
            ...
            <div style={styles.chips}>
              <button
                type="button"
                onClick={() => {
                  router.push(buildUrl("personal", new Date(startDate).toISOString(), null));
                }}
                style={{
                  ...styles.chip,
                  background:
              ...
              >
                <span style={{ ...styles.chipDot, background: "rgba(250,204,21,0.95)" }} />
                Personal
              </button>
              ...
              <button
                type="button"
                onClick={() => {
                  const gid = selectedGroupId || activeGroupId || (uniqueGroups[0]?.id ?? "");
                  router.push(buildUrl("group", new Date(startDate).toISOString(), gid || null));
                }}
                style={{
                  ...styles.chip,
                  background:
                    effectiveType === "group" ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.03)",
                }}
              >
            ...
            <div style={{ ...styles.field, marginTop: 12 }}>
              ...
              {loadingGroups || booting ? (
                ...
                <div style={styles.emptyInline}>
                  ...
                  <div style={styles.emptyInlineSub}>Crea uno para poder hacer eventos compartidos.</div>
                  <button onClick={() => router.push("/groups/new")} style={styles.primaryBtnSmall}>
                    Crear grupo
                  </button>
                </div>
              ) : (
                ...
                >
                  {uniqueGroups.map((g) => (
                    <option key={g.id} value={g.id}>
                      {g.name ?? "Grupo"} ({g.type === "family" ? "Familia" : "Pareja"})
                    </option>
                  ))}
                </select>
              )}
...
function ConflictPreflightModal({
  ...
  return (
    <div style={modalStyles.wrap}>
      <button style={modalStyles.backdrop} onClick={onClose} aria-label="Cerrar" />
      <div style={modalStyles.card}>
        <div style={modalStyles.header}>
          ...
          <div style={modalStyles.p}>
            ...
            <b style={{ opacity: 0.9 }}>{count}</b> evento{count === 1 ? "" : "s"}.
          </div>
        </div>
                ...
                <div style={modalStyles.itemTop}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={modalStyles.overlapPill}>
                      Se cruza: {new Date(it.overlapStart).toLocaleString()} —{" "}
        ...
        <div style={modalStyles.tip}>Tip: esto respeta tus Settings (warn-before-save + default-resolution).</div>
      </div>
    </div>
  );
}
...
function ChoiceCard({
  ...
  return (
    <button
      ...
      style={{
        ...modalStyles.choice,
        borderColor: active ? "rgba(52,211,153,0.22)" : "rgba(255,255,255,0.12)",
        background: active ? "rgba(52,211,153,0.08)" : "rgba(255,255,255,0.05)",
      }}
    >
...
const modalStyles: Record<string, React.CSSProperties> = {
  ...
  badgeDot: { width: 9, height: 9, borderRadius: 99, background: "rgba(248,113,113,0.95)" },
  h2: { marginTop: 10, fontSize: 20, fontWeight: 950, letterSpacing: "-0.2px" },
  p: { marginTop: 6, fontSize: 13, opacity: 0.75, lineHeight: 1.4 },
  listBox: { padding: "0 18px 14px" },
  listInner: {
    maxHeight: 260,
    overflow: "auto",
    borderRadius: 16,
  ...
  itemSub: { marginTop: 4, fontSize: 12, opacity: 0.7, fontWeight: 700 },
  overlapPill: {
    marginTop: 8,
    display: "inline-flex",
    padding: "6px 10px",
  ...
  choiceTitle: { fontSize: 13, fontWeight: 950 },
  choiceDesc: { marginTop: 4, fontSize: 12, opacity: 0.7, fontWeight: 700 },
  footer: {
    padding: "0 18px 16px",
    display: "flex",
  ...
  tip: { padding: "0 18px 16px", fontSize: 11, opacity: 0.55, fontWeight: 700 },
};
...
const styles: Record<string, React.CSSProperties> = {
  ...
  heroTitleRow: { display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" },
  h1: { margin: 0, fontSize: 28, letterSpacing: "-0.6px" },
  heroSub: { fontSize: 13, opacity: 0.75, maxWidth: 520, lineHeight: 1.4 },
  pill: {
    display: "inline-flex",
    alignItems: "center",
    gap: 8,
  ...
  card: { borderRadius: 18, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", padding: 14 },
  ...
  input: { width: "100%", padding: "12px 12px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(6,10,20,0.55)", color: "rgba(255,255,255,0.92)", outline: "none", fontSize: 14 },
  textarea: { width: "100%", padding: "12px 12px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(6,10,20,0.55)", color: "rgba(255,255,255,0.92)", outline: "none", fontSize: 14, resize: "vertical" },
  select: { width: "100%", padding: "12px 12px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.10)", background: "rgba(6,10,20,0.55)", color: "rgba(255,255,255,0.92)", outline: "none", fontSize: 14 },
  skeleton: { padding: 14, borderRadius: 14, border: "1px dashed rgba(255,255,255,0.16)", background: "rgba(255,255,255,0.02)", opacity: 0.75, fontSize: 13 },
  emptyInline: { padding: 14, borderRadius: 16, border: "1px dashed rgba(255,255,255,0.16)", background: "rgba(255,255,255,0.02)" },
  emptyInlineTitle: { fontWeight: 900, fontSize: 13 },
  emptyInlineSub: { marginTop: 6, opacity: 0.75, fontSize: 12 },
  primaryBtnSmall: { marginTop: 10, padding: "10px 12px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.14)", background: "linear-gradient(135deg, rgba(56,189,248,0.20), rgba(124,58,237,0.20))", color: "rgba(255,255,255,0.95)", cursor: "pointer", fontWeight: 900 },
  hint: { fontSize: 12, opacity: 0.72 },
  grid2: { display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12, marginTop: 8 },
  errorBox: { marginTop: 14, borderRadius: 16, border: "1px solid rgba(248,113,113,0.28)", background: "rgba(248,113,113,0.10)", padding: 12 },
  ...
  footerRow: { marginTop: 14, display: "flex", gap: 10, justifyContent: "space-between", flexWrap: "wrap" },
  ...
  primaryBtn: { padding: "10px 14px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.14)", background: "linear-gradient(135deg, rgba(56,189,248,0.20), rgba(124,58,237,0.20))", color: "rgba(255,255,255,0.95)", cursor: "pointer", fontWeight: 900 },
  ghostBtnWide: { padding: "12px 14px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.92)", cursor: "pointer", fontWeight: 900, minWidth: 240 },
  primaryBtnWide: { padding: "12px 14px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.14)", background: "linear-gradient(135deg, rgba(56,189,248,0.22), rgba(124,58,237,0.22))", color: "rgba(255,255,255,0.95)", cursor: "pointer", fontWeight: 900, minWidth: 240 },
};
