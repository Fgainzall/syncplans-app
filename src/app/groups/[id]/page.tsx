...
export default function GroupDetailsPage() {
  ...
  const canSend =
    trimmedEmail.includes("@") && trimmedEmail.length >= 6 && !sending;
  ...
  async function sendInvite() {
    ...
    try {
      setSending(true);
      setLastInviteLink(null); // limpiamos link previo
      ...
      // ID normalizado de la invitación
      ...
      const emailSent =
        typeof res?.email_sent === "boolean" ? res.email_sent : null;
  ...
  async function handleSaveMeta() {
    ...
    try {
      ...
      showToast(
        "Grupo actualizado ✅",
        "Todos verán el nuevo nombre y tipo."
      );
    } catch (e: any) {
      showToast(
  ...
  async function handleDeleteGroup() {
    ...
    const ok = window.confirm(
      "Vas a eliminar este grupo, sus eventos y mensajes para todos. ¿Seguro?"
    );
    if (!ok) return;
  ...
  async function handleLeaveGroup() {
    ...
    if (isOwner) {
      showToast(
        "Eres el owner",
        "El owner no puede salir del grupo; puede eliminarlo."
      );
      return;
    }
    ...
    const ok = window.confirm(
      "Vas a salir de este grupo y dejarás de ver sus eventos y mensajes. ¿Seguro?"
    );
    if (!ok) return;
      ...
      <div style={styles.shell}>
        ...
        <section style={styles.card}>
          ...
          <div style={styles.smallNote}>
            Cambia el nombre o el tipo del grupo. El cambio aplica para todos
            los miembros.{" "}
            {isOwner
            ...
            >
              <button
                ...
                style={{
                  ...styles.primaryBtn,
                  opacity:
                    !isOwner || !hasGroupMetaChanges ? 0.55 : 1,
                }}
              >
        ...
        <section style={styles.card}>
          ...
          <div style={styles.smallNote}>
            Personas que ven este calendario compartido. Cuando alguien acepta
            una invitación, aparecerá aquí automáticamente.
          </div>
          ...
          <div style={styles.table}>
            ...
            {membersLoading ? (
              <div
                style={{ padding: 12, opacity: 0.75, fontSize: 12 }}
              >
                Cargando miembros…
              </div>
            ) : members.length === 0 ? (
        ...
        <section style={styles.card}>
          ...
          <div style={styles.smallNote}>
            Un espacio ligero para coordinar detalles sobre planes con este
            grupo. Los mensajes solo los ven los miembros; no reemplaza tu
            WhatsApp, solo acompaña al calendario compartido.
          </div>
          ...
          <div style={styles.messagesWrap}>
            {messagesLoading ? (
              ...
              <div style={{ fontSize: 12, opacity: 0.75 }}>
                Aún no hay mensajes. Escribe el primero para coordinar algo
                rápido (horarios, lugar, quién trae qué, etc.).
              </div>
            ) : (
              messages.map((msg) => {
                const name = msg.isMe
        ...
        <section style={styles.card}>
          ...
          <div style={styles.smallNote}>
            ...
            <b>pending</b> y, cuando la acepte, se agregará automáticamente
          ...
          {lastInviteLink && (
            ...
            >
              ...
              <div style={{ fontSize: 11, opacity: 0.7 }}>
                Compártelo por WhatsApp, correo o donde prefieras. Cuando lo
                abra, verá la pantalla para aceptar la invitación.
              </div>
            </div>
          )}
        </section>
